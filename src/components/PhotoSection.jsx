import { useState } from "react";
import { useAuth } from "../context/AuthContext";
import PhotoList from "./PhotoList";
import { supabase } from "../lib/supabase";
import ConfirmModal from "./ConfirmModal";
import { usePhoto } from "../context/PhotoContext";

const PhotoSection = ({ section, setPhotoData }) => {
  const { user } = useAuth();
  const {loading, error, setConfirmDeleteSection} = usePhoto();

  return (
    <section className="relative flex flex-col gap-8">
      <div className="flex flex-col items-center text-center gap-4">
        <h2 className="text-3xl md:text-4xl font-bold text-accgray">
          {section.name}
        </h2>
        <div className="w-20 h-1.5 bg-linear-to-r from-accblue to-accgreenlight rounded-full"></div>
        <p className="text-accgray/70 max-w-xl">{section.caption}</p>
        {error && <p className="text-red-500 italic">{error}</p>}
      </div>
      <div className="bg-white/30 backdrop-blur-sm p-4 rounded-3xl border border-white/50 shadow-xl">
        <PhotoList section={section} />
      </div>
      {user?.role === "ADMIN" && section.media.length === 0 && (
        <button
        type="button"
        onClick={() => setConfirmDeleteSection(section)}
        disabled={loading}
        className="absolute top-10 right-0 text-red-500 hover:text-red-700 hover:cursor-pointer active:scale-125 transition-colors duration-300 ease-in-out"
        >
          {loading ? (
            <div className="w-10 h-10 rounded-full border-4 border-accgray border-t-red-500 animate-spin"></div>
          ) : (
            <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth="2"
            stroke="currentColor"
            className="size-8"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
              />
            </svg>
          )}
        </button>
      )}
    </section>
  );
};

export default PhotoSection;
